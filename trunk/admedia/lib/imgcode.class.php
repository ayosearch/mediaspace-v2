...
class PM_ImgCode {
	...
	function PM_ImgCode() {
		list($w,$h,$n) = explode("\t",$GLOBALS['cfg_icsize']);
		...
		$this->height	= $h;
		$this->num		= (int)$n;
		$this->style	= $GLOBALS['cfg_icstyle'];
		$this->gdtype	= $GLOBALS['cfg_ictype'];
	...
	function background() {
		...
		if (($this->style & 8) && function_exists('imagecreatefromjpeg') && function_exists('imagecopymerge')) {
			if ($fp = @opendir(R_P.$GLOBALS["cfg_imgpath"].'/vc/bg/')) {
				while ($flie = @readdir($fp)) {
					if (preg_match('/\.jpg$/i',$flie)) {
						$bgs[] = R_P.$GLOBALS["cfg_imgpath"].'/vc/bg/'.$flie;
					}
				}
				@closedir($fp);
			}
	...
	function getCode($type=null) {
		empty($type) && $type = $this->gdtype;
		$code = '';
		switch ($type) {
			case 2:
				global $cfg_charset,$lang;
				require_once GetLang('vc');
				$step = strtoupper($cfg_charset) == 'UTF-8' ? 3 : 2;
				$len  = (strlen($lang['vc'])/$step) - 1;
				for ($i = 0; $i < $this->num; $i++) {
					$code .= substr($lang['vc'],mt_rand(0,$len)*$step,$step);
				}
				$this->code=$code;				
				if (strtoupper($cfg_charset) <> 'UTF-8') {
					$code = $this->convert($code,'UTF-8',$cfg_charset);
				}
	...
	function ttffont(&$im) {
		global $db_gdtype;
		$codefont = R_P.$GLOBALS["cfg_imgpath"].($this->gdtype == 2 ? '/fonts/ch/' : '/fonts/en/');
	...
	function imgfont(&$im) {
		...
		if (function_exists('imagecreatefromgif')) {
			$imgfont = R_P.$GLOBALS["cfg_imgpath"].'/vc/gif/';
